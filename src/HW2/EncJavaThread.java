...
public class EncJavaThread extends Thread{
	static long start;     
    ...
    public EncJavaThread(int index) 
    {
    	this.index = index;  
    }
    public void run() {
    	...
    	messageByteBlock = Arrays.copyOfRange(messageBytes, index*16, ((index+1)*16));
    	encrypt(index, keyStr, messageByteBlock);      	
    ...
    public static void main(String[] args) throws Exception {
        ...
        EncJavaThread threadler[] = new EncJavaThread[messageBytes.length/16];
        
        for (int i = 0; i < threadler.length; i++) {             
        	threadler[i] = new EncJavaThread(i);         
        }                  
        for (int i = 0; i < threadler.length; i++) {             
        	threadler[i].start();         
        }
